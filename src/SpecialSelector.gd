...
func update_list(location: Location) -> bool:
	...
	$VBoxContainer/StartAutoDestruct.visible = location.id == Globals.Location.COMMAND_CENTER and Globals.self_destruct_activated == false
	$VBoxContainer/StopAutoDestruct.visible = location.id == Globals.Location.COMMAND_CENTER  and Globals.self_destruct_activated
